...
export async function postJson(path, payload) {
  const res = await fetch(`${API_BASE}${path}`, {
    // send a network request to API_BASE + path
  ...
  if (!res.ok) {
    // if HTTP status is not in the 200-299 range, treat as error
    // Try to parse JSON error, otherwise fallback to text
    try {
      // attempt to parse the error payload as JSON
